...
class TestSquaredLoss(unittest.TestCase):
    def test_SquaredLoss_simple(self):
        ...
        t.call("squared_loss")
        # check that the result array contains the correct output
        t.check_array(arr2, [36, 4, 1, 0, 64, 1, 1, 4])
        # check that the register a0 contains the correct output
        t.check_scalar("a0", 111)
        # generate the `assembly/TestSquaredLoss_test_simple.s` file and run it through venus
        t.execute()

    def test_squaredLoss_invalid_length(self):
        # load the test for abs_loss.s
        t = AssemblyTest(self, "../coverage-src/squared_loss.s")
...
class TestInitializeZero(unittest.TestCase):
    def test_InitialzeZero_simple(self):
    ...
    def test_InitializeZero_invalid_length(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")

        # set a0 to an invalid length of the array
        t.input_scalar("a0", 0)
        # call the `initialize_zero` function
        t.call("initialize_zero")
        t.execute(code=36)

    def test_InitializeZero_malloc_fail(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        t.input_scalar("a0", 5)
        t.call("initialize_zero")
